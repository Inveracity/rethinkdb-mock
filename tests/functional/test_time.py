...
class TestDateTimeGetters(MockTest):
    ...
    def test_year(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].year())
            .run(conn)
        )
    ...
    def test_month(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].month())
            .run(conn)
        )
    ...
    def test_day(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].day())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))
    ...
    def test_hours(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].hours())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))
    ...
    def test_minutes(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].minutes())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))
    ...
    def test_seconds(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].seconds())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))

    def test_time_of_day(self, conn):
        expected = set([((((12 * 60) + 10) * 60) + 32), ((((17 * 60) + 3) * 60) + 54)])
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].time_of_day())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))

    def test_day_of_week(self, conn):
        expected = set([1, 2])  # 2014 August 25 -> Monday  # 2014 June 3 -> Tuesday
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].day_of_week())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))
    ...
    def test_date(self, conn):
        ...
        result = (
            r.db("d")
            .table("people")
            .map(lambda doc: doc["last_updated"].date())
            .run(conn)
        )
        assertEqual(expected, set(list(result)))
...
class TestMoreTime(MockTest):
        ...
        data = [
            {
                ...
                "last_updated": datetime.datetime(
                    2014, 6, 3, 0, 0, 1, tzinfo=r.make_timezone("00:00")
                ),
            },
            {
                "id": "sam",
                "last_updated": datetime.datetime(
                    2014, 8, 25, 0, 0, 0, tzinfo=r.make_timezone("00:00")
                ),
            },
        ]
        return as_db_and_table("d", "people", data)
    ...
    def test_epoch_time(self, conn):
        results = (
            r.db("d")
            .table("people")
            .map(lambda d: d.merge({"as_epoch": d["last_updated"].to_epoch_time()}))
            .run(conn)
        )
...
class TestTime(MockTest):
    ...
    def test_error_with_no_timezone(self, conn):
        ...
        try:
            r.db("unimportant").table("very").update({"update_time": date}).run(conn)
...
class TestDuring(MockTest):
    ...
    def test_during_1(self, conn):
        expected = [{"id": "joe", "is_during": False}, {"id": "sam", "is_during": True}]
        result = (
            r.db("d")
            .table("people")
            .map(
                lambda doc: {
                    ...
                    "is_during": doc["last_updated"].during(
                        r.time(2014, 7, 10, "Z"), r.time(2014, 12, 1, "Z")
                    ),
                }
            )
            .run(conn)
    ...
    def test_during_2(self, conn):
        expected = [{"id": "joe", "is_during": True}, {"id": "sam", "is_during": False}]
        result = (
            r.db("d")
            .table("people")
            .map(
                lambda doc: {
                    "id": doc["id"],
                    "is_during": doc["last_updated"].during(
                        r.time(2014, 5, 10, "Z"), r.time(2014, 7, 1, "Z")
                    ),
                }
            )
            .run(conn)
        ...
        result = (
            ...
            .map(
                lambda doc: {
                    "id": doc["id"],
                    "is_during": doc["last_updated"].during(
                        r.time(2014, 6, 3, "Z"), r.time(2014, 8, 25, "Z")
                    ),
                }
            )
            .run(conn)
    ...
    def test_filter_during(self, conn):
        ...
        result = table.filter(
            lambda doc: doc["last_updated"].during(
                r.time(2014, 6, 1, "Z"), r.time(2014, 6, 30, "Z")
            )
        ).run(conn)
        result = list(result)
