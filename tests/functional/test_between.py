...
class TestBetween(MockTest):
    ...
    def test_between_id_default_range(self, conn):
        ...
        result = r.db("s").table("people").between("bob", "zuul").run(conn)
    ...
    def test_between_id_closed_right(self, conn):
        ...
        result = (
            r.db("s")
            .table("people")
            .between("bob", "tom", right_bound="closed")
            .run(conn)
        )
        result = list(result)
    ...
    def test_between_id_open_left(self, conn):
        expected = [{"id": "joe", "first_name": "Joseph", "last_name": "Smith"}]
        result = (
            r.db("s").table("people").between("bob", "tom", left_bound="open").run(conn)
        )
        result = list(result)
        assertEqUnordered(expected, result)
    ...
    def test_between_id_open_left_closed_right(self, conn):
        ...
        result = (
            r.db("s")
            .table("people")
            .between("bob", "tom", left_bound="open", right_bound="closed")
            .run(conn)
        )
        result = list(result)
        assertEqUnordered(expected, result)
    ...
    def test_between_index_default_range(self, conn):
        ...
        result = (
            r.db("s")
            .table("people")
            .between("Builder", "Smith", index="last_name")
            .run(conn)
        )
        result = list(result)
        assertEqUnordered(expected, result)
    ...
    def test_between_index_closed_right(self, conn):
        ...
        result = (
            r.db("s")
            .table("people")
            .between("Builder", "Smith", index="last_name", right_bound="closed")
            .run(conn)
        )
        result = list(result)
        assertEqUnordered(expected, result)
