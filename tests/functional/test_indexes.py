...
class TestIndexes(MockTest):
    ...
    def test_func_index_create(self, conn):
        ...
        r.db("s").table("people").index_create(
            "first_and_last", lambda doc: doc["first_name"] + doc["last_name"]
        ).run(conn)
        result = r.db("s").table("people").index_list().run(conn)
    ...
    def test_func_index_create_works(self, conn):
        ...
        result = (
            r.db("s")
            .table("people")
            .get_all("BobBuilder", "TomGeneric", index="first_and_last")
            .run(conn)
        )
        assertEqUnordered(expected, list(result))
    ...
    def test_index_rename_works(self, conn):
        ...
        indexes = list(r.db("s").table("people").index_list().run(conn))
        assertEqual(["last_name"], indexes)
        r.db("s").table("people").index_rename("last_name", "new_last_name").run(conn)
        indexes = list(r.db("s").table("people").index_list().run(conn))
        assertEqual(["new_last_name"], indexes)

    def test_index_rename_works_2(self, conn):
        expected = [{"id": "tom", "first_name": "Tom", "last_name": "Generic"}]
        r.db("s").table("people").index_create("last_name").run(conn)
        indexes = list(r.db("s").table("people").index_list().run(conn))
        assertEqual(["last_name"], indexes)
        r.db("s").table("people").index_rename("last_name", "new_last_name").run(conn)
        ...
        result = (
            r.db("s")
            .table("people")
            .get_all("Generic", index="new_last_name")
            .run(conn)
        )
    ...
    def test_index_wait_one_works(self, conn):
        ...
        result = (
            r.db("s").table("people").get_all("Builder", index="last_name").run(conn)
        )
        assertEqual(expected, list(result))
...
class TestIndexUpdating(MockTest):
    ...
    def test_index_update(self, conn):
        ...
        people.index_create("last_name").run(conn)
        people.index_wait().run(conn)
        people.insert({"id": "someone", "first_name": "Some", "last_name": "One"}).run(
            conn
        )
        result = list(people.get_all("One", index="last_name").run(conn))
