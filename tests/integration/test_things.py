...
from tests.integration import MockTest
...
class TestThings(MockTest):
    ...
    def test_join_filter_map(self, conn):
        query = (
            r.db("x")
            .table("employees")
            .eq_join("person", r.db("x").table("people"))
            .filter(lambda p: p["right"]["age"] > 20)
            .map(lambda d: d["left"].merge({"person": d["right"]["name"]}))
        )
        expected = [
            {"id": "joe-employee-id", "person": "joe", "job": "lawyer-id"},
            {"id": "tim-employee-id", "person": "tim", "job": "nurse-id"},
        ]
    ...
    def test_index_getall_map_orderby(self, conn):
        r.db("x").table("people").index_create(
            "name_and_id", lambda doc: doc["name"] + doc["id"]
        ).run(conn)
        ...
        query = (
            r.db("x")
            .table("people")
            .get_all("joejoe-id", "timtim-id", index="name_and_id")
            .map(
    ...
    def test_multi_join(self, conn):
        query = (
            ...
            .eq_join("person", r.db("x").table("people"))
            .map(lambda d: d["left"].merge({"person": d["right"]["name"]}))
            .eq_join("job", r.db("x").table("jobs"))
            .map(lambda d: d["left"].merge({"job": d["right"]["name"]}))
        )
        expected = [
            {"id": "joe-employee-id", "person": "joe", "job": "Lawyer"},
            {"id": "tim-employee-id", "person": "tim", "job": "Nurse"},
            {"id": "bob-employee-id", "person": "bob", "job": "Assistant"},
            {"id": "todd-employee-id", "person": "todd", "job": "Lawyer"},
        ]
        assertEqUnordered(expected, list(query.run(conn)))
